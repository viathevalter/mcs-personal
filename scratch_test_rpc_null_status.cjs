...
async function testSearchWorkers() {
    const { data: allWithNullFilter, error } = await prodSupabase
        .schema('core_personal')
        .rpc('search_workers', {
            p_empresa_id: null,
            p_search: null,
            p_cliente_nombre: null,
            p_status_trabajador_filter: null,
    ...
    console.log("Total search_workers with NULL status filter:", allWithNullFilter?.length, error?.message || '');

    const targetNames = ["ARLINTON", "LUIS MIGUEL VEGA", "NICXON", "GABRIEL CORREIA"];
    const found = allWithNullFilter?.filter(w => targetNames.some(tn => w.nome.includes(tn)));
    console.log("Found in search_workers:", found?.map(w => `${w.nome} (status: ${w.status_trabajador})`));
}
