...
async function run() {
    ...
    try {
        ...
        console.log(`Total workers returned by RPC: ${workers.length}`);

        const counts = {};
        for (const w of workers) {
            counts[w.contratante] = (counts[w.contratante] || 0) + 1;
        }
        console.log("Breakdown of contratante for these workers:", counts);
