...
const migrationSql = `
-- Drop old or create in public
CREATE OR REPLACE FUNCTION public.fn_get_campaigns_stats(p_empresa_id UUID DEFAULT NULL)
RETURNS TABLE (
  campaign_id UUID,
  total BIGINT,
  sent BIGINT,
...
GRANT EXECUTE ON FUNCTION public.fn_get_campaigns_stats(UUID) TO authenticated, anon, service_role;

-- Also keep in core_comercial
...
async function apply(name, connStr) {
  ...
  try {
    ...
    console.log(`Applying public.fn_get_campaigns_stats on ${name}...`);
    await client.query(migrationSql);

    // Test function directly
    const res = await client.query(`SELECT * FROM public.fn_get_campaigns_stats();`);
    console.log(`Test call on ${name} returned ${res.rows.length} campaigns.`);
    console.table(res.rows.slice(0, 5));

    console.log(`Successfully created public.fn_get_campaigns_stats on ${name}!`);
  } catch (err) {
