...
async function fetchHub(province, keywords, excludedCompanies) {
...
async function runStep() {
  ...
  try {
    ...
    if (jobRes.rows.length === 0) {
      console.log('💤 All missions completed! Queue is clear.');
      await client.end();
      return { active: false };
    }

    const job = jobRes.rows[0];
    console.log(`\n🚀 [DAEMON] Processing Mission: "${job.title}" | Current leads: ${job.found_emails_count}/${job.target_count}`);
    ...
    console.log(`🌐 Dispatching 8 Parallel Provincial Hubs: ${targetHubs.map(h => h.split(' ')[0]).join(', ')}`);
    ...
    console.log(`📥 Received ${candidates.length} candidates. Verifying DNS MX servers in parallel...`);
    ...
    console.log(`✅ Verified ${verifiedRecords.length} fresh leads with live DNS MX! Saving to database...`);
    ...
    let isCompleted = currentCount >= job.target_count || (verifiedRecords.length <= 1 && currentCount >= 40);
    if (isCompleted) {
      console.log(`🎉 Mission "${job.title}" is COMPLETED with ${currentCount} leads! Advancing queue...`);
      await client.query(`
        UPDATE core_comercial.lead_prospecting_jobs 
      ...
      console.log(`📈 Mission "${job.title}" updated: ${currentCount}/${job.target_count} leads.`);
      await client.query(`
        UPDATE core_comercial.lead_prospecting_jobs 
    ...
    await client.end();
    return { active: true, newLeads: verifiedRecords.length, total: currentCount };
  } catch (err) {
    console.error('❌ Error in daemon step:', err);
...
async function loop() {
  console.log('========================================================================');
  console.log('🤖 24/7 AUTONOMOUS CLOUD PROSPECTOR DAEMON RUNNING IN BACKGROUND');
  console.log('========================================================================');

  while (true) {
    try {
      const res = await runStep();
      // 1 second pause between batches for smooth rate limiting
