...
async function run() {
    ...
    try {
        ...
        // Fetch allocations where contratante matches STOCCO
        const res = await client.query(`
            ...
            WHERE COALESCE(va.contratante, c.contratante) = 'STOCCO'
              AND (
                 -- Worker is active
        ...
        console.log(`Active Workers count by contratante = STOCCO: ${res.rows.length}`);
