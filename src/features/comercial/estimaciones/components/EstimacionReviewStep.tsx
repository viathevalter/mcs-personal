...
export function EstimacionReviewStep({ data }: Props) {
  ...
  return (
    <div className="space-y-6">
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium text-muted-foreground flex items-center">
                <CheckCircle2 className="h-4 w-4 mr-2" /> Validação Técnica
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex justify-between border-b pb-2">
                <span className="text-sm">Total de Trabalhadores:</span>
                <span className="font-medium">{totalWorkers}</span>
              </div>
              <div className="flex justify-between border-b pb-2">
                <span className="text-sm">Volume de Horas (Estimado):</span>
                <span className="font-medium">{totalHours}h</span>
              </div>
              <div className="flex justify-between pb-2">
                <span className="text-sm">Quantidade de Perfis/Funções:</span>
                <span className="font-medium">{data.items.length}</span>
              </div>
              
              {data.costs.length > 0 && (
                <div className="mt-4 pt-4 border-t">
                  <span className="text-sm text-muted-foreground block mb-2">Custos Adicionais Declarados:</span>
                  <div className="flex flex-wrap gap-2">
                    {data.costs.map((c: any) => (
                      <span key={c.id} className="text-xs bg-slate-100 px-2 py-1 rounded-full border">
          ...
          <Card className="bg-slate-50 border-2">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium flex items-center">
                <DollarSign className="h-4 w-4 mr-2 text-blue-600" /> Viabilidade Financeira
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex justify-between text-sm text-slate-500">
                <span>Custo Base Estimado:</span>
                <span className="font-medium">{formatCurrency(data.total_estimated_cost)}</span>
              </div>
              <div className="flex justify-between text-base font-bold text-blue-700 pb-2 border-b">
                <span>Receita Total Estimada:</span>
                <span>{formatCurrency(data.total_estimated_revenue)}</span>
              </div>
              
              <div className="pt-2">
                <span className="text-sm font-medium text-slate-500 block mb-1">Margem Global Calculada:</span>
                <div className="flex items-center space-x-3">
                  <span className={`text-3xl font-black tracking-tight ${
                    data.estimated_margin_percent >= 20 ? 'text-emerald-600' :
                    data.estimated_margin_percent >= 10 ? 'text-amber-500' : 'text-red-500'
                  }`}>
                    {data.estimated_margin_percent}%
                  </span>
                  
                  {isMarginLow && (
                    <div className="flex items-center text-xs text-amber-600 bg-amber-50 px-2 py-1 rounded-md border border-amber-200">
      ...
      <div className="bg-blue-50/50 p-4 rounded-lg border border-blue-100 text-sm text-blue-800 flex items-start mt-6">
        <Info className="h-5 w-5 text-blue-500 mr-3 shrink-0" />
        <p>
