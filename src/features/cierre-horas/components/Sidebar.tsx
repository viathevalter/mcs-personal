...
import { Box, UploadCloud, List, ArrowLeft, Menu } from 'lucide-react';
    ...
    <aside className={`${isSidebarOpen ? 'w-64' : 'w-20'} bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 h-screen fixed left-0 top-0 flex flex-col z-20 overflow-y-auto overflow-x-hidden transition-all duration-300`}>
        ...
        <div className="flex items-center gap-3">
          ...
          {isSidebarOpen && (
            <div className="min-w-0">
              <h1 className="text-sm font-bold text-slate-900 dark:text-slate-100 leading-tight truncate">Cierre de Horas</h1>
              <p className="text-[10px] text-slate-500 dark:text-slate-400 font-medium truncate">Módulo de Extração</p>
            </div>
          )}
        </div>
        <button
      ...
      <div className="flex-1 px-3 py-2">
        ...
        <div className="space-y-1">
          <NavItem to="/cierre-horas/upload" icon={UploadCloud} label="Importar Horas" />
          <NavItem to="/cierre-horas/logs" icon={List} label="Histórico OCR" />
        </div>
      </div>
    </aside>
  );
