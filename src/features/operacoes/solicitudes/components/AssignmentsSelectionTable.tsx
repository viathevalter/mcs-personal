      ...
      <Table>
        <TableHeader className="sticky top-0 bg-slate-50/95 dark:bg-slate-900/95 backdrop-blur-sm z-10">
          <TableRow>
            ...
            {!isCompact && <TableHead className="px-1.5 py-2 text-xs md:text-sm">Obra Atual (Local)</TableHead>}
            {!isCompact && <TableHead className="whitespace-nowrap px-1.5 py-2 text-xs md:text-sm">Início</TableHead>}
            {!isCompact && <TableHead className="whitespace-nowrap px-1.5 py-2 text-xs md:text-sm">Tipo</TableHead>}
            {!isCompact && <TableHead className="px-1.5 py-2 text-xs md:text-sm">Cadeia</TableHead>}
          </TableRow>
        </TableHeader>
        <TableBody>
          {assignments.map((assignment) => {
            ...
            return (
              ...
              >
                ...
                {isCompact ? (
                  <TableCell className="px-1.5 py-1.5 text-xs leading-tight">
                    <div className="font-semibold text-slate-800 dark:text-slate-200">{clientName}</div>
                    <div className="text-[11px] text-muted-foreground">{clientSiteName}</div>
                  </TableCell>
                ) : (
