...
MCS - Gestão Comercial`;
  ...
  const handleExportHoursPDFFatura = async (fatura: any) => {
    ...
    const clientName = fatura.clientName || fatura.client?.legal_name || fatura.client?.razon_social || fatura.client?.nombre_comercial || 'Cliente';
    ...
    faturaHours.forEach(h => {
      ...
      wObj.horasDiarias[dateKey] = (wObj.horasDiarias[dateKey] || 0) + Number(h.horas_totais || 0);
    });
    ...
    // Build tables to render (separated by Obra if multiple obras exist)
    const tablesToRender: Array<{
      title: string;
      workers: any[];
      totalHoras: number;
      totalValor: number;
      ...
      {
        title: fatura.ajustes_json?.obra ? `OBRA: ${fatura.ajustes_json.obra.toUpperCase()}` : 'OBRA: TODAS AS OBRAS',
    ...
    const obrasMap = new Map<string, { id: string; name: string }>();
    faturaHours.forEach((h: any) => {
      if (h.obra_id) {
        obrasMap.set(h.obra_id, { id: h.obra_id, name: h.obra_name || h.obra?.name || 'Obra' });
      }
    });

    if (obrasMap.size > 1) {
      obrasMap.forEach(obra => {
        const oWorkersMap = new Map();
        faturaHours.filter((h: any) => h.obra_id === obra.id).forEach((h: any) => {
          const wId = h.worker_id;
          if (!wId) return;
          if (!oWorkersMap.has(wId)) {
            oWorkersMap.set(wId, {
              workerId: wId,
              workerName: h.worker?.nome || 'Colaborador',
              horasDiarias: {}
          ...
          const dateKey = h.data_trabalho.includes('T') ? h.data_trabalho.split('T')[0] : h.data_trabalho;
          oWorkersMap.get(wId).horasDiarias[dateKey] = (oWorkersMap.get(wId).horasDiarias[dateKey] || 0) + Number(h.horas_totais || 0);
        });

        const oWorkers = Array.from(oWorkersMap.values());
        if (oWorkers.length > 0) {
          const oTotalHoras = faturaHours.filter((h: any) => h.obra_id === obra.id).reduce((sum: number, h: any) => sum + Number(h.horas_totais || 0), 0);
          const oTotalValor = faturaHours.filter((h: any) => h.obra_id === obra.id).reduce((sum: number, h: any) => sum + (Number(h.horas_totais || 0) * Number(h.tarifa_faturada || 0)), 0);
          tablesToRender.push({
            title: `OBRA: ${obra.name.toUpperCase()}`,
            workers: oWorkers,
            totalHoras: oTotalHoras,
            totalValor: oTotalValor
          });
        }
      });
    }
    ...
    tablesToRender.forEach((table) => {
      ...
      while (currentIndex < workers.length) {
        ...
        pageDiv.className = 'pdf-page-hours-modal';
          ...
          headerHtml = `
            <div style="display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 2px solid #e2e8f0; padding-bottom: 12px; margin-bottom: 12px;">
              ...
              <div style="text-align: right;">
                <p style="font-size: 13px; color: #64748b; margin: 0;">Total de Horas: <strong style="color: #1e293b; font-size: 16px;">${table.totalHoras.toFixed(2)}h</strong></p>
                <p style="font-size: 13px; color: #64748b; margin: 4px 0 0 0;">Faturamento Base: <strong style="color: #1e293b; font-size: 16px;">€ ${table.totalValor.toLocaleString('pt-PT', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong></p>
              </div>
            </div>
    ...
    try {
      ...
      const pageElements = container.querySelectorAll('.pdf-page-hours-modal');
  ...
  const generateHoursPDFAttachment = async (f: ClientBillingSummary): Promise<{ name: string, contentType: string, contentBytes: string } | null> => {
    ...
    tablesToRender.forEach((table) => {
      ...
      while (currentIndex < workers.length) {
        ...
        const pageDiv = document.createElement('div');
        pageDiv.className = 'pdf-page-hours-attachment';
        pageDiv.style.width = '1120px';
        pageDiv.style.height = '792px';
          ...
          headerHtml = `
            <div style="display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 2px solid #e2e8f0; padding-bottom: 12px; margin-bottom: 12px;">
              ...
              <div style="text-align: right;">
                ...
                <p style="font-size: 13px; color: #64748b; margin: 4px 0 0 0;">${labels.baseBilling}: <strong style="color: #1e293b; font-size: 16px;">€ ${displayTotalValor.toLocaleString('pt-PT', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong></p>
              </div>
            </div>
            <div style="text-align: center; font-weight: 800; font-size: 13px; letter-spacing: 0.05em; background-color: #f1f5f9; padding: 8px; color: #334155; border-radius: 6px; margin-bottom: 12px; border: 1px solid #e2e8f0;">
              ${table.title}
    ...
    try {
      ...
      const pageElements = container.querySelectorAll('.pdf-page-hours-attachment');
  ...
  const handleExportHoursPDF = async (f: ClientBillingSummary) => {
    ...
    tablesToRender.forEach((table) => {
      ...
      while (currentIndex < workers.length) {
        ...
        const pageDiv = document.createElement('div');
        pageDiv.className = 'pdf-page-hours';
        pageDiv.style.width = '1120px';
        pageDiv.style.height = '792px';
        pageDiv.style.padding = '40px';
        pageDiv.style.boxSizing = 'border-box';
          ...
          headerHtml = `
            <div style="display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 2px solid #e2e8f0; padding-bottom: 12px; margin-bottom: 12px;">
              ...
              <div style="text-align: right;">
                ...
                <p style="font-size: 13px; color: #64748b; margin: 4px 0 0 0;">Faturamento Base: <strong style="color: #1e293b; font-size: 16px;">€ ${f.totalValor.toLocaleString('pt-PT', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong></p>
              </div>
            </div>
            <div style="text-align: center; font-weight: 800; font-size: 13px; letter-spacing: 0.05em; background-color: #f1f5f9; padding: 8px; color: #334155; border-radius: 6px; margin-bottom: 12px; border: 1px solid #e2e8f0;">
              ${table.title}
    ...
    try {
      ...
      const pageElements = container.querySelectorAll('.pdf-page-hours');
                        ...
                        <div id={`factura-sheet-${selectedFatura.id}`} className="w-[800px] min-w-[800px] max-w-[800px] h-[1130px] bg-white p-8 pb-20 border border-slate-200 text-slate-800 rounded text-left relative flex flex-col justify-between select-none shadow-md">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="grid grid-cols-3 gap-6 mb-8 text-[11px] leading-relaxed border-t border-slate-100 pt-4">
                              ...
                              <div>
                                <p className="font-bold text-[9px] text-[#ec8a5e] uppercase mb-1">De</p>
                                <p className="font-bold text-slate-900">{selectedFatura.empresaNome}</p>
                                <p className="text-slate-600">{selectedFatura.empresaAddressLine || 'N/A'}</p>
                                <p className="text-slate-600">{[selectedFatura.empresaPostalCode, selectedFatura.empresaCity].filter(Boolean).join(' ')}</p>
                                <p className="text-slate-600">{selectedFatura.empresaProvince || 'Portugal'}</p>
                                {selectedFatura.empresaEmail && <p className="text-slate-600">{selectedFatura.empresaEmail}</p>}
                                <p className="text-slate-600">Nº Contribuinte: {selectedFatura.empresaTaxId || 'N/A'}</p>
                                {selectedFatura.empresaCapitalSocial && <p className="text-slate-600">Capital Social: {selectedFatura.empresaCapitalSocial}</p>}
                                {selectedFatura.empresaConservatoria && <p className="text-slate-600">Cons. Reg. Com.: {selectedFatura.empresaConservatoria}</p>}
                                {selectedFatura.empresaMatricula && <p className="text-slate-600">Matrícula: {selectedFatura.empresaMatricula}</p>}
                              </div>

                              {/* Coluna 2: Detalhes */}
                              <div>
                                <p className="font-bold text-[9px] text-[#ec8a5e] uppercase mb-1">ATCUD</p>
                                <p className="font-semibold text-slate-900">{selectedFatura.atcud || `${selectedFatura.empresaAtcudPrefix || 'J6XBVVRV'}-${selectedFatura.empresaNextInvoiceNumber || 1}`}</p>
                              ...
                              <div>
                                ...
                                <p className="text-slate-600 mt-2">Nº Contribuinte: {selectedFatura.taxId || 'N/A'}</p>
                              </div>
                            </div>
                          ...
                          <div className="space-y-4">
                            {/* Certified software text */}
                            <div className="text-center text-[10px] text-slate-700 italic font-semibold">
                              {selectedFatura.empresaCertifiedSoftwareText || 'Dclm - Processado por Programa Certificado nº 1137/AT'}
                            </div>
                            ...
                            <div className="border-t border-slate-200 pt-3 flex justify-between text-[9px] text-slate-500 font-medium">
                              <div>
                                <p className="font-bold uppercase mb-0.5">Local de Carga</p>
                                <p>{selectedFatura.empresaAddressLine || 'N/ Morada'}</p>
                                <p>{[selectedFatura.empresaPostalCode, selectedFatura.empresaCity].filter(Boolean).join(' ')}</p>
                              </div>
                              {adjustments.iban && (
                                <div className="text-center">
                                  <p className="font-bold uppercase mb-0.5">Informações de Pagamento</p>
                              ...
                              <div className="text-right">
                                <p className="font-bold uppercase mb-0.5">Local de Descarga</p>
                                <p>{selectedFatura.clientAddressLine || 'V/ Morada'}</p>
                                <p>{[selectedFatura.clientPostalCode, selectedFatura.clientCity, selectedFatura.clientCountryName].filter(Boolean).join(', ')}</p>
                              </div>
                            </div>
                          </div>
