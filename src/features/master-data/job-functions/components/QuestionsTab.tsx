...
import type { CreateJobFunctionQuestionDTO } from '../types';
...
export function QuestionsTab({ jobFunctionId }: QuestionsTabProps) {
  ...
  const { createQuestion, isCreating, archiveQuestion } = useMutateJobFunctionQuestion(jobFunctionId);
  ...
  const onSubmit = async (data: CreateJobFunctionQuestionDTO) => {
    try {
      await createQuestion({ ...data, sort_order: questions.length * 10 });
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="flex justify-end space-x-2 pt-2 border-t">
                ...
                <Button type="submit" disabled={isCreating}>
                  {isCreating ? 'Salvando...' : 'Salvar Pergunta'}
                </Button>
              </div>
            </form>
          </Form>
