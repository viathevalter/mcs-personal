-- Migration: 20260708140000_auto_complete_date_change_solicitudes.sql
-- Description: Automatically set status to completed for order_postponement, order_extension, and order_termination, and automatically complete their tasks.

-- 1. Criar ou substituir a função para processar as solicitações antes de salvar
CREATE OR REPLACE FUNCTION core_operacoes.trigger_pre_processar_solicitud()
RETURNS TRIGGER AS $$
BEGIN
    -- Se for adiamento de início, prorrogação ou finalização de obra, o status deve ser completed
    IF NEW.tipo IN ('order_postponement', 'order_extension', 'order_termination') THEN
...
-- 2. Recriar trigger BEFORE INSERT OR UPDATE
DROP TRIGGER IF EXISTS tgr_solicitudes_operativas_pre_processar ON core_operacoes.solicitudes_operativas;
CREATE TRIGGER tgr_solicitudes_operativas_pre_processar
    BEFORE INSERT OR UPDATE ON core_operacoes.solicitudes_operativas
    FOR EACH ROW
    EXECUTE FUNCTION core_operacoes.trigger_pre_processar_solicitud();

-- 3. Atualizar a função do trigger AFTER UPDATE para AFTER INSERT OR UPDATE
...
BEGIN
    ...
    -- Se for concluída, concluir todas as tarefas dela automaticamente
    IF NEW.status = 'completed' AND NEW.tipo IN ('order_postponement', 'order_extension', 'order_termination') THEN
        UPDATE core_operacoes.solicitud_tareas
        SET status = 'completed', completed_at = NOW(), updated_at = NOW()
        WHERE solicitud_id = NEW.id AND status != 'completed';
    END IF;

    -- Se a solicitud é de adiamento de início e acabou de ser concluída (ou inserida como concluída)
    IF NEW.tipo = 'order_postponement' AND NEW.status = 'completed' AND (TG_OP = 'INSERT' OR OLD.status != 'completed') THEN
    ...
    -- Se a solicitud é de prorrogação e acabou de ser concluída (ou inserida como concluída)
    ELSIF NEW.tipo = 'order_extension' AND NEW.status = 'completed' AND (TG_OP = 'INSERT' OR OLD.status != 'completed') THEN
    ...
    -- Se a solicitud é de finalização e acabou de ser concluída (ou inserida como concluída)
    ELSIF NEW.tipo = 'order_termination' AND NEW.status = 'completed' AND (TG_OP = 'INSERT' OR OLD.status != 'completed') THEN
...
-- 4. Recriar o trigger AFTER INSERT OR UPDATE
...
CREATE TRIGGER tgr_solicitudes_operativas_notificar
    AFTER INSERT OR UPDATE ON core_operacoes.solicitudes_operativas
    FOR EACH ROW
