-- 1. Redefine core_personal.alocar_trabalhador_em_vaga to support replacement allocations without hitting capacity limit
...
BEGIN
    ...
    -- 8. Atualizar pedido_item (SE FOR REEMPLAZO, A QUANTIDADE PREENCHIDA LÍQUIDA NÃO ALTERA, POIS SUBSTUÍMOS UM)
    ...
    -- 9. Recalcular operational_status do Pedido Inteiro (apenas se mudou a quantidade preenchida)
    IF v_solicitud_id IS NULL THEN
        SELECT 
            COUNT(*),
    ...
    IF v_solicitud_id IS NOT NULL THEN
        -- 1. Buscar a alocação original (source)
        ...
        -- 2. Terminar a alocação original no banco (usando status 'replaced' compatível com o check constraint)
        IF v_source_assignment_id IS NOT NULL THEN
            UPDATE core_personal.worker_assignments
            SET status = 'replaced',
                end_date = COALESCE(v_planned_start_date - 1, CURRENT_DATE),
                updated_at = NOW(),
                updated_by = v_user_id
            WHERE id = v_source_assignment_id;
        END IF;

        -- 3. Registrar na Timeline da Solicitação
        ...
        -- 4. Atualizar o target correspondente para 'completed' e vincular a nova alocação e trabalhador de destino
        ...
        -- 5. Se todos os targets desta solicitação foram completados, atualizar o status da solicitação cabeçalho para 'completed'
        IF NOT EXISTS (
            SELECT 1 
            FROM core_operacoes.solicitud_targets 
            WHERE solicitud_id = v_solicitud_id 
    ...
    RETURN jsonb_build_object(
        ...
        'operational_status', COALESCE(v_new_operational_status, 'fulfilled'),
        'item_status', COALESCE(v_item_status, 'fulfilled'),
