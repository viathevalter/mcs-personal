-- ========================================================================================
-- Migration: 20260525160000_add_country_id_to_estimaciones_and_pedidos.sql
-- Description: Add country_id to estimaciones and pedidos tables, update criar_estimacion_completa and aprovar_estimacion functions.
-- ========================================================================================

BEGIN;

-- 1. Adicionar colunas country_id
ALTER TABLE core_comercial.estimaciones ADD COLUMN IF NOT EXISTS country_id UUID REFERENCES core_common.countries(id) ON DELETE RESTRICT;
ALTER TABLE core_comercial.pedidos ADD COLUMN IF NOT EXISTS country_id UUID REFERENCES core_common.countries(id) ON DELETE RESTRICT;

-- 2. Criar índices para performance
CREATE INDEX IF NOT EXISTS idx_estimaciones_country ON core_comercial.estimaciones(country_id);
CREATE INDEX IF NOT EXISTS idx_pedidos_country ON core_comercial.pedidos(country_id);

-- 3. Atualizar a função criar_estimacion_completa
...
-- 4. Atualizar a função aprovar_estimacion
...
BEGIN
    ...
    -- 8. Criar o Pedido (inserindo o country_id copiado da estimación)
