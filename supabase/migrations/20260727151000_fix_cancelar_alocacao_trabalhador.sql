-- Fix cancelar_alocacao_trabalhador to support standalone operational replacements (where pedido_item_id is NULL)
...
BEGIN
    ...
    -- Buscar código do pedido principal para histórico nas observações se existir
    ...
    -- 2. Se a alocação estava ativa/planejada e possui pedido_item_id, reabrir a vaga correspondente no pedido item
    IF v_status_anterior IN ('planned', 'active', 'paused') AND v_solicitud_id IS NULL AND v_pedido_item_id IS NOT NULL THEN
        UPDATE core_comercial.pedido_items
        SET quantity_fulfilled = GREATEST(0, quantity_fulfilled - 1)
        WHERE id = v_pedido_item_id;
    END IF;

    -- 3. Recalcular o status operacional do Pedido Principal se existir
    IF v_pedido_id IS NOT NULL THEN
    ...
    SET status_trabajador = 'Desistiu',
        ...
        notes = COALESCE(notes || E'\n', '') || 'Desistiu da contratação em ' || to_char(now(), 'DD/MM/YYYY HH24:MI') || COALESCE(': ' || v_reason, '')
    ...
    IF v_solicitud_id IS NOT NULL THEN
        ...
        WHERE solicitud_id = v_solicitud_id 
          AND (target_assignment_id = v_assignment_id OR target_assignment_id IS NULL);
