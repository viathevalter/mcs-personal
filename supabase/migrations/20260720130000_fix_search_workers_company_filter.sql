...
BEGIN
    ...
    WITH current_allocations AS (
        ...
        WHERE 
          (cpp.fechainiciopedido IS NULL OR cpp.fechainiciopedido <= CURRENT_DATE)
          AND (cpp.fechasalidatrabajador IS NULL OR cpp.fechasalidatrabajador >= CURRENT_DATE)
          AND (cpp.fechafinpedido IS NULL OR cpp.fechafinpedido >= CURRENT_DATE)
    ),
    base_workers AS (
        SELECT 
            w.id,
        ...
        WHERE 
            -- Resolve company membership globally, by active/planned allocation in public.colaborador_por_pedido, or fallback to contracts/assignments
            (
                ...
                OR (
                    NOT EXISTS (
                        SELECT 1 FROM current_allocations ca 
                        WHERE ca.cod_colab = w.cod_colab
                    )
                    AND (
                        EXISTS (
                            SELECT 1 FROM core_personal.contracts cnt 
                            WHERE cnt.worker_id = w.id AND cnt.empresa_id = p_empresa_id
                              AND cnt.created_at::date <= CURRENT_DATE
                              AND (cnt.terminated_at IS NULL OR cnt.terminated_at::date >= CURRENT_DATE)
