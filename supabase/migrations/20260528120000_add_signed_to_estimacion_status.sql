-- ========================================================================================
-- Migration: 20260528120000_add_signed_to_estimacion_status.sql
-- Description: Add 'signed' status to core_comercial.estimaciones and allow it in aprovar_estimacion function.
-- ========================================================================================

BEGIN;

-- 1. Alterar a constraint CHECK da tabela core_comercial.estimaciones
ALTER TABLE core_comercial.estimaciones DROP CONSTRAINT IF EXISTS estimaciones_status_check;
ALTER TABLE core_comercial.estimaciones ADD CONSTRAINT estimaciones_status_check CHECK (status IN ('draft', 'review', 'sent', 'signed', 'approved', 'rejected', 'expired', 'superseded', 'cancelled'));

-- 2. Atualizar a função aprovar_estimacion para aceitar o status 'signed'
...
BEGIN
    ...
    -- 3. Validar estado da estimación (adicionado 'signed' na validação)
    ...
    -- 9. Clonar os Itens para o Pedido (incluindo ss_regime e custom_lodging_rate)
