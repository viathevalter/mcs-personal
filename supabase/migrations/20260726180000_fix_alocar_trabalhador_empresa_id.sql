-- Fix alocar_trabalhador_em_vaga by removing jf.empresa_id from the job_functions join
...
BEGIN
    ...
    ELSE
        ...
        LEFT JOIN core_comercial.job_functions jf ON UPPER(TRIM(jf.name)) = UPPER(TRIM(w.funcion))
        WHERE w.id = (
