-- Redefine core_personal.search_workers to resolve company membership by active contracts or allocations, 
-- and dynamically show current contratante, funcion and cliente based on the active/planned allocation.
...
BEGIN
    ...
    WITH base_workers AS (
        SELECT 
            ...
            COALESCE(
                (
                    SELECT wa.job_function_name_snapshot 
                    FROM core_personal.worker_assignments wa 
                    WHERE wa.worker_id = w.id AND wa.status IN ('planned', 'active', 'paused') 
                    ORDER BY wa.planned_start_date DESC LIMIT 1
                ),
            ...
            COALESCE(
                ...
                w.cliente, 
                core_personal.fn_get_active_client_for_worker(w.cod_colab)
            ) as active_client_nombre,
            w.data_ingresso,
            w.data_baixa,
...
-- Redefine core_personal.get_client_worker_kpis to also include company membership resolved by allocations,
-- and dynamic fields inside calculations.
