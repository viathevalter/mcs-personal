-- ==============================================================================
-- Migração Evolutiva - Bloco 3: Registro General (Master Data) - VERSÃO 2.2
-- ==============================================================================
...
-- Limpeza Prévias Idempotente APENAS para tabelas deste escopo
DO $$ DECLARE
    r RECORD;
BEGIN
    FOR r IN (
        ...
        FROM pg_policies 
        WHERE (schemaname = 'core_common' AND tablename IN ('countries', 'regions', 'clients', 'client_sites', 'suppliers', 'audit_logs'))
           OR (schemaname = 'core_comercial' AND tablename IN ('job_functions', 'job_function_questions', 'job_function_rate_refs'))
           OR (schemaname = 'core_logistica' AND tablename IN ('epis', 'job_function_epis'))
    ) LOOP
...
-- Escrita Protegida (UPDATE - com WITH CHECK para proteger vazamento de empresa_id)
CREATE POLICY "Edicao de clientes" ON "core_common"."clients" FOR UPDATE TO authenticated USING (core_common.has_role(empresa_id, 'super_admin') OR core_common.has_role(empresa_id, 'admin_rh') OR core_common.has_role(empresa_id, 'operador')) WITH CHECK (core_common.has_role(empresa_id, 'super_admin') OR core_common.has_role(empresa_id, 'admin_rh') OR core_common.has_role(empresa_id, 'operador'));
CREATE POLICY "Edicao de obras" ON "core_common"."client_sites" FOR UPDATE TO authenticated USING (core_common.has_role(empresa_id, 'super_admin') OR core_common.has_role(empresa_id, 'admin_rh') OR core_common.has_role(empresa_id, 'operador')) WITH CHECK (core_common.has_role(empresa_id, 'super_admin') OR core_common.has_role(empresa_id, 'admin_rh') OR core_common.has_role(empresa_id, 'operador'));
CREATE POLICY "Edicao de fornecedores" ON "core_common"."suppliers" FOR UPDATE TO authenticated USING (core_common.has_role(empresa_id, 'super_admin') OR core_common.has_role(empresa_id, 'admin_rh') OR core_common.has_role(empresa_id, 'operador')) WITH CHECK (core_common.has_role(empresa_id, 'super_admin') OR core_common.has_role(empresa_id, 'admin_rh') OR core_common.has_role(empresa_id, 'operador'));
CREATE POLICY "Edicao de funcoes" ON "core_comercial"."job_functions" FOR UPDATE TO authenticated USING (core_common.has_role(empresa_id, 'super_admin') OR core_common.has_role(empresa_id, 'admin_rh')) WITH CHECK (core_common.has_role(empresa_id, 'super_admin') OR core_common.has_role(empresa_id, 'admin_rh'));
CREATE POLICY "Edicao de perguntas" ON "core_comercial"."job_function_questions" FOR UPDATE TO authenticated USING (core_common.has_role(empresa_id, 'super_admin') OR core_common.has_role(empresa_id, 'admin_rh')) WITH CHECK (core_common.has_role(empresa_id, 'super_admin') OR core_common.has_role(empresa_id, 'admin_rh'));
CREATE POLICY "Edicao de custos" ON "core_comercial"."job_function_rate_refs" FOR UPDATE TO authenticated USING (core_common.has_role(empresa_id, 'super_admin') OR core_common.has_role(empresa_id, 'admin_rh')) WITH CHECK (core_common.has_role(empresa_id, 'super_admin') OR core_common.has_role(empresa_id, 'admin_rh'));
CREATE POLICY "Edicao de epis" ON "core_logistica"."epis" FOR UPDATE TO authenticated USING (core_common.has_role(empresa_id, 'super_admin') OR core_common.has_role(empresa_id, 'operador')) WITH CHECK (core_common.has_role(empresa_id, 'super_admin') OR core_common.has_role(empresa_id, 'operador'));
CREATE POLICY "Edicao de epis_funcao" ON "core_logistica"."job_function_epis" FOR UPDATE TO authenticated USING (core_common.has_role(empresa_id, 'super_admin') OR core_common.has_role(empresa_id, 'admin_rh')) WITH CHECK (core_common.has_role(empresa_id, 'super_admin') OR core_common.has_role(empresa_id, 'admin_rh'));
...
-- Constraints de Unicidade Parciais Adicionais (Conforme V2.2)
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_clients_codigo_active ON "core_common"."clients"("empresa_id", "codigo") WHERE "codigo" IS NOT NULL AND "status" != 'archived';
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_suppliers_codigo_active ON "core_common"."suppliers"("empresa_id", "codigo") WHERE "codigo" IS NOT NULL AND "status" != 'archived';

CREATE UNIQUE INDEX IF NOT EXISTS uq_client_sites_code_active ON "core_common"."client_sites"("empresa_id", "client_id", "site_code") WHERE "site_code" IS NOT NULL AND "status" != 'archived';

CREATE UNIQUE INDEX IF NOT EXISTS uq_job_functions_code_active ON "core_comercial"."job_functions"("empresa_id", "code") WHERE "code" IS NOT NULL AND "status" != 'archived';
