-- ========================================================================================
-- Migration: 20260916190000_add_commercial_review_governance.sql
-- Description: Add governance review fields to estimaciones and RPC for manager decision.
-- ========================================================================================

BEGIN;

-- 1. Alterar tabela estimaciones para incluir campos de auditoria de análise/aprovação
ALTER TABLE core_comercial.estimaciones
ADD COLUMN IF NOT EXISTS review_justification TEXT,
ADD COLUMN IF NOT EXISTS review_requested_at TIMESTAMPTZ,
ADD COLUMN IF NOT EXISTS review_decision_notes TEXT,
ADD COLUMN IF NOT EXISTS reviewed_by UUID REFERENCES auth.users(id),
ADD COLUMN IF NOT EXISTS reviewed_at TIMESTAMPTZ,
ADD COLUMN IF NOT EXISTS viability_reasons JSONB;

-- 2. Atualizar RPC criar_estimacion_completa para persistir os campos de revisão
...
DECLARE
    ...
    v_review_justification TEXT;
    v_review_requested_at TIMESTAMPTZ;
    v_viability_reasons JSONB;
...
BEGIN
    ...
    v_review_justification := p_payload->>'review_justification';
    IF p_payload->>'review_requested_at' IS NOT NULL THEN
        v_review_requested_at := (p_payload->>'review_requested_at')::timestamptz;
    ELSIF v_status = 'review' THEN
        v_review_requested_at := NOW();
    ELSE
        v_review_requested_at := NULL;
    END IF;
    v_viability_reasons := p_payload->'viability_reasons';
    ...
    IF NOT (
        ...
        core_common.has_role(v_empresa_id, 'operador') OR
    ...
    INSERT INTO core_comercial.estimaciones (
        ...
        review_justification, review_requested_at, viability_reasons,
        ...
        v_review_justification, v_review_requested_at, v_viability_reasons,
        FALSE,
        v_user_id, v_user_id
    )
    ...
    INSERT INTO core_comercial.estimacion_versions (
        ...
        v_empresa_id, v_estimacion_id, 1, 'draft',
    ...
    LOOP
        ...
        INSERT INTO core_comercial.estimacion_items (
            ...
            quantity, planned_hours_per_day, planned_days_per_week, total_hours,
            includes_accommodation, includes_transport, includes_ppe,
            base_cost_hour, recommended_sell_rate, minimum_sell_rate, sell_rate_hour,
            margin_percent, risk_level, notes,
            created_by, updated_by
        ) VALUES (
            ...
            COALESCE((v_item->>'includes_accommodation')::boolean, false), 
            COALESCE((v_item->>'includes_transport')::boolean, false), 
    ...
    SET 
        ...
        estimated_margin_percent = v_margin_percent
    WHERE id = v_version_id;

    -- Atualizar totais na Estimación
    UPDATE core_comercial.estimaciones
    SET 
        total_estimated_cost = v_total_estimated_cost,
        total_estimated_revenue = v_total_estimated_revenue,
...
-- 3. Atualizar RPC atualizar_estimacion_completa para suportar status review e salvar justificação
...
BEGIN
    ...
    IF v_status_check NOT IN ('draft', 'review') THEN
        RAISE EXCEPTION 'Apenas estimaciones em status rascunho (draft) ou sob revisão (review) podem ser editadas.';
    END IF;
    ...
    -- Atualizar Estimación (se for editada, reseta a aprovação do gerente)
    UPDATE core_comercial.estimaciones
    SET 
        ...
        review_justification = COALESCE(v_review_justification, review_justification),
        review_requested_at = COALESCE(v_review_requested_at, review_requested_at),
        viability_reasons = COALESCE(v_viability_reasons, viability_reasons),
        total_estimated_cost = v_total_estimated_cost,
        total_estimated_revenue = v_total_estimated_revenue,
        estimated_margin_percent = v_margin_percent,
        updated_at = NOW(),
...
-- 4. Criar RPC decidir_aprovacao_gerente
CREATE OR REPLACE FUNCTION core_comercial.decidir_aprovacao_gerente(
    p_estimacion_id UUID,
    p_aprovado BOOLEAN,
...
BEGIN
    ...
    -- Validar permissão: admin ou super_admin
    IF NOT (
        core_common.has_role(v_empresa_id, 'super_admin') OR 
        core_common.has_role(v_empresa_id, 'admin')
    ) THEN
        RAISE EXCEPTION 'Apenas gerentes e administradores podem aprovar ou rejeitar exceções de orçamentos.';
    END IF;

    IF p_aprovado THEN
        -- Ao aprovar, move para draft liberando a emissão da proposta com flag is_approved_by_manager = TRUE
        v_new_status := 'draft';
        UPDATE core_comercial.estimaciones
        SET 
            status = v_new_status,
            ...
            reviewed_at = NOW(),
            review_decision_notes = p_decision_notes,
            updated_at = NOW(),
            updated_by = v_user_id
        WHERE id = p_estimacion_id;
    ELSE
        -- Ao rejeitar, marca status = 'rejected'
...
-- Permissões
REVOKE ALL ON FUNCTION core_comercial.decidir_aprovacao_gerente(UUID, BOOLEAN, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION core_comercial.decidir_aprovacao_gerente(UUID, BOOLEAN, TEXT) TO authenticated;
