-- Migration: 20260724180000_fix_replacement_flow.sql
-- Description: Improve replacement code generation sequence, client mapping on standalone replacements, and fix alocar_trabalhador_em_vaga bugs.

-- 1. Redefine core_operacoes.criar_solicitud_operativa_com_targets with R-YYYY-000NNN format starting at 221
...
BEGIN
    ...
    -- 1.5 Gerar código da Solicitud Operativa (SOL-YYYY-000001 ou R-YYYY-000NNN)
    IF v_type = 'replacement' THEN
        SELECT COUNT(*) INTO v_solicitudes_count
        FROM core_operacoes.solicitudes_operativas
        WHERE empresa_id = v_empresa_id 
          AND tipo = 'replacement';
          
        v_solicitud_codigo := 'R-' || TO_CHAR(NOW(), 'YYYY') || '-' || LPAD((v_solicitudes_count + 221)::TEXT, 6, '0');
    ELSE
        SELECT COUNT(*) INTO v_solicitudes_count
        FROM core_operacoes.solicitudes_operativas
        WHERE empresa_id = v_empresa_id 
...
-- 2. Redefine core_personal.alocar_trabalhador_em_vaga with standalone replacement support & fix typo bugs
...
BEGIN
    ...
    ELSE
        ...
        -- Get job function from original worker function
        SELECT 
            w.funcion,
            jf.id
        INTO
        ...
        -- If target actually has a source_pedido_item_id, reload job_function details from it
        IF v_pedido_item_id IS NOT NULL THEN
            SELECT 
                job_function_id, 
                COALESCE(NULLIF(TRIM(pi.job_function_name_snapshot), ''), NULLIF(TRIM(jf.name), ''), 'Desconhecida')
