-- ========================================================================================
-- Migration: 20260519000000_rpc_alocar_trabalhador.sql
-- Description: Criação da RPC para Alocação Inicial de Trabalhadores em Vagas (Pedido Item)
-- ========================================================================================
...
BEGIN
    ...
    -- 5. Lidar com o Worker (Criar se não existir)
    IF v_worker_id IS NULL THEN
        ...
        INSERT INTO core_personal.workers (id, empresa_id, nome, nif, status_trabajador)
        VALUES (v_worker_id, v_empresa_id, v_worker_name, v_worker_document, 'Ativo');
    ELSE
    ...
    -- Geramos o UUID no DECLARE para já podermos usar no root_assignment_id
    INSERT INTO core_personal.worker_assignments (
        ...
        v_source_id,
        v_notes,
        v_new_assignment_id, -- root_assignment_id é ele mesmo, pois é a origem
        v_user_id
    );
    ...
    ELSE
        -- "Se ainda faltar gente, manter status = 'active' ou 'pending', conforme o padrão atual"
        -- Se já tivermos alguém alocado, consideramos ativo
    ...
    IF v_fulfilled_items = 0 THEN
        -- Verifica se tem algum item active/partially fulfilled
