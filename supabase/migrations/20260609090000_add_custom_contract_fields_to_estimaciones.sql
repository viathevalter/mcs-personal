-- Migration: 20260609090000_add_custom_contract_fields_to_estimaciones.sql
-- Description: Add custom contract fields and update versioning RPC

BEGIN;

-- 1. Adicionar as colunas se não existirem
ALTER TABLE core_comercial.estimaciones
ADD COLUMN IF NOT EXISTS custom_contract_url TEXT,
ADD COLUMN IF NOT EXISTS is_custom_contract_approved BOOLEAN DEFAULT FALSE,
ADD COLUMN IF NOT EXISTS custom_contract_uploaded_at TIMESTAMP WITH TIME ZONE,
ADD COLUMN IF NOT EXISTS custom_contract_uploaded_by UUID;

-- 2. Atualizar a stored procedure core_comercial.criar_nova_versao_estimacion para gerenciar os novos campos
