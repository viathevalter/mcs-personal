-- Migration: 20260708160000_unify_sequential_codes.sql
-- Description: Unifies the sequential auto-generation formatting to 6 digits, updates estimaciones creation to use it directly, and ensures orders and operative requests share the exact same numeric code of the originating estimation.

BEGIN;

-- 1. Redefine next_estimacion_code function to use 6 digits (Formato: EST-YYYY-000001)
...
BEGIN
    ...
    RETURN 'EST-' || current_year || '-' || lpad(next_val::text, 6, '0');
END;
$$ LANGUAGE plpgsql;

-- 2. Redefine next_pedido_code function to use 6 digits (Formato: PED-YYYY-000001)
...
BEGIN
    ...
    RETURN 'PED-' || current_year || '-' || lpad(next_val::text, 6, '0');
END;
$$ LANGUAGE plpgsql;

-- 3. Redefine criar_estimacion_completa to use sequential generation (no more random characters)
...
-- 4. Redefine aprovar_estimacion to inherit the exact same code number from the estimation
CREATE OR REPLACE FUNCTION core_comercial.aprovar_estimacion(
...
REVOKE ALL ON FUNCTION core_comercial.aprovar_estimacion(UUID, DATE, DATE) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION core_comercial.aprovar_estimacion(UUID, DATE, DATE) TO authenticated;
