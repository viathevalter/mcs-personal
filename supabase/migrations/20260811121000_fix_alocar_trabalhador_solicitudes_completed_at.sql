-- Description: Redefine core_personal.alocar_trabalhador_em_vaga to use updated_at instead of completed_at on core_operacoes.solicitudes_operativas.
...
BEGIN
    ...
    ELSE
        ...
        IF v_pedido_item_id IS NOT NULL THEN
            ...
            IF v_existing_active_assignment IS NOT NULL THEN
                RAISE EXCEPTION 'O trabalhador já possui uma alocação activa neste pedido.';
            END IF;
        END IF;
    ...
    IF v_solicitud_id IS NOT NULL THEN
        ...
        IF NOT EXISTS (
            ...
            SET status = 'completed',
                updated_at = now() -- CORRIGIDO DE completed_at PARA updated_at
