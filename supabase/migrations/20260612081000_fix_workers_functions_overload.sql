-- 1. Drop all potentially conflicting functions to clear overloads
DROP FUNCTION IF EXISTS core_personal.get_client_worker_kpis(uuid, text);
...
DROP FUNCTION IF EXISTS core_personal.search_workers(uuid, text, text[], text[], text[], text, text, text, text, integer, integer, integer, integer);

-- 2. Recreate get_client_worker_kpis with correct text[] signature
...
-- 3. Recreate search_workers with correct text[] and period month/year signatures
