...
fs.writeFileSync(path.join(__dirname, 'supabase', 'migrations', '20260902200000_comercial_power_dialer_and_call_logs.sql'), sql.trim(), 'utf8');

async function run() {
  for (const [name, connStr] of [['DEV', devConnectionString], ['PROD', prodConnectionString]]) {
    const client = new Client({ connectionString: connStr });
    try {
      await client.connect();
      console.log(`Connected to ${name} DB.`);
      await client.query(sql);
      console.log(`Successfully applied dialer migration on ${name} DB!`);
    } catch (err) {
      console.error(`Error applying to ${name} DB:`, err);
    } finally {
      await client.end();
    }
  }
