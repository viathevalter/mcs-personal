...
async function runOnDb(name, connStr, sql) {
  ...
  try {
    ...
    console.log(`Successfully applied migration to ${name} DB!`);
  } catch (err) {
...
async function main() {
  const sqlPath = path.join(__dirname, 'supabase', 'migrations', '20260924000000_operacoes_reunioes_wbr_pdca.sql');
