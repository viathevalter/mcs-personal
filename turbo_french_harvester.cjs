...
async function harvestLoop() {
  ...
  console.log("====================================================================");
  console.log("⚡ TURBO FRENCH HARVESTER (8 MISSÕES PARALELAS - DADOS 100% REAIS)");
  console.log("====================================================================");

  // Activate all 8 French missions
  ...
  console.log("📊 Exclusão inicial: " + existingNames.size + " empresas / " + existingEmails.size + " e-mails conhecidos.");
  ...
  while (true) {
    for (const mapping of MISSION_NAF_MAPPING) {
      ...
      for (const naf of mapping.nafCodes) {
        console.log("  📂 Consultando NAF Oficial " + naf + "...");
        ...
        for (let page = 1; page <= 30; page++) {
          try {
            ...
            // Process all 25 companies concurrently in parallel
            ...
            for (const lead of validLeads) {
              ...
              console.log("    ✓ [LEAD REAL] " + lead.company_name + " | " + lead.website + " | " + lead.email + " | " + (lead.phone || "N/A"));
            }
    ...
    console.log("\n🔄 Ciclo concluído. Recomeçando busca contínua...");
    await new Promise(r => setTimeout(r, 2000));
  }
}

harvestLoop().catch(console.error);
