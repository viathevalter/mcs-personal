...
async function printAllClientSums() {
    ...
    const workers = await fetchAllSchema('core_personal', 'workers', 'cod_colab, status_trabajador', { col: 'empresa_id', val: EMPRESA_ID });
    ...
    for (const w of workers) {
        const wClient = activeClientMap[w.cod_colab] || 'SEMC_CLIENTE';
        const s = (w.status_trabajador || '').toUpperCase();
        if (s.includes('ATIVO') || s.includes('ACTIVO')) {
            if (!s.includes('INATIVO') && !s.includes('INACTIVO')) {
                clientCounts[wClient] = (clientCounts[wClient] || 0) + 1;
            }
        }
    }

    console.log('--- TODOS OS CLIENTES COM ATIVOS ---');
    for (const [c, count] of Object.entries(clientCounts)) {
        console.log(`Cliente: ${c} | Ativos: ${count}`);
    }
}
